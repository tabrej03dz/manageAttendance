...
<div class="p-4 sm:p-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200">
        <div class="p-4 sm:p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Create Letter Template</h1>
                <p class="text-sm text-gray-500 mt-1">
                    Use dynamic variables like @{{employee_name}}, @{{designation}}, @{{salary}}
                </p>
            </div>
        ...
        <form action="{{ route('letter-templates.store') }}" method="POST" class="p-4 sm:p-6">
            @csrf
            @include('letter_templates.form', ['letterTemplate' => null])
        </form>
    </div>
</div>
@endsection
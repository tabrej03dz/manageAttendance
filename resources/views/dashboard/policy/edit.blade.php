            ...
            <div class="container-fluid">
                <div class="card shadow-lg border-danger rounded-3 mb-4">
                    <div class="card-header bg-danger text-white text-center rounded-top">
                        <h2 class="mb-0 py-3">Edit Policy</h2>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('policy.update', ['policy' => $policy->id]) }}" method="POST">
                                        ...
                                        <select name="office_id" id="office_id" class="form-control">
                                            ...
                                            @foreach ($offices as $office)
                                                <option value="{{ $office->id }}"
                                                    {{ $policy->office_id == $office->id ? 'selected' : '' }}>
                                                    {{ $office->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="description" class="form-control" id="description" cols="30" rows="2">{{ $policy->description }}</textarea>
                                </div>
                            </div>
                            <div class="row mb-3">

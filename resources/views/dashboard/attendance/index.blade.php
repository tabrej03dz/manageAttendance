                        ...
                        <table class="min-w-full bg-white rounded-lg shadow-sm">
                            ...
                            @endphp
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $off ? $d->format('d-[D]').' '. $off?->title.' (OFF)' : $d->format('d-[D]')}}</td>
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $off ? $d->format('d-[D]').' '. $off?->title.' (OFF)' : $d->format('d-[D]')}}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $record?->check_in?->format('h:i:s A') }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $record?->late ? App\Http\Controllers\HomeController::getTime($record->late) : 'N/A' }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">
                                    @if($record)
                                        <img src="{{asset('storage/'.$record?->check_in_image)}}" alt="Check-in"
                                                class="w-10 h-10 rounded-full">
                                    @endif
                                    </td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $record?->check_out?->format('h:i:s A') }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">
                                    @if($record)
                                        <img src="{{asset('storage/'.$record?->check_out_image)}}" alt="Check-in"
                                                class="w-10 h-10 rounded-full">
                                    @endif
                                   </td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{ $record?->duration ? App\Http\Controllers\HomeController::getTime($record->duration) : '' }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{$record?->day_type}}</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{round($record?->check_out_distance)}} m</td>
                                    <td class="px-4 py-4 text-sm text-gray-700">{{round($record?->check_out_distance)}} m</td>
                                </tr>
                                @endif
                                @endforeach

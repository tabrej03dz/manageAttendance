    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <form action="{{ route('attendance.index', ['user' => $user?->id]) }}" method="GET" class="d-inline-block ml-2">
                @csrf
                <input type="date" name="start" placeholder="Start Date" class="form-control d-inline-block mb-2 mb-sm-0" style="width: auto;">
                <input type="date" name="end" placeholder="End Date" class="form-control d-inline-block mb-2 mb-sm-0" style="width: auto;">
                <input type="submit" value="Filter" class="btn btn-success mb-2 mb-sm-0">
            </form>
            <a href="{{ route('attendance.form', ['form_type' => 'check_in']) }}" class="btn btn-primary ml-2 mb-2 mb-sm-0">Check In</a>
            <a href="{{ route('attendance.form', ['form_type' => 'check_out']) }}" class="btn btn-danger ml-2 mb-2 mb-sm-0">Check Out</a>
            @role('super_admin')
                @foreach($users as $u)
            ...
            <div class="table-responsive mt-3">
                <table class="table table-striped table-bordered">
                    ...
                    @foreach ($dates as $dateObj)
                        ...
                        @endphp
                        <tr style="background-color: {{$d->format('D') == 'Sun' ? '#fbd9d3' : ''}};">
                            <td>{{ $d->format('d-[D]') }}</td>
                            ...
                            <td>
                                @if($record?->check_in_image)
                                    <a href="{{ asset('storage/' . $record->check_in_image) }}" target="_blank">
                                        <img src="{{ asset('storage/' . $record->check_in_image) }}" alt="Check-in Image" class="img-fluid" style="max-width: 100px;">
                                    </a>
                                @endif
                            </td>
